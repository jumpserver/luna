...
import { WebProxyInteraction, buildInteractionGuardScript, INTERACTION_WORLD } from "../web-proxy/interaction";
import { WebProxyScript, installWebProxyNavigationGuard } from "../web-proxy/script";
...
async function completeWebProxyVerification(managed) {
  ...
  if (!(await interaction.complete()) || !managed.autofillPending || managed.interaction !== interaction) return false;
...
async function checkWebProxyLoginSuccess(managed) {
  ...
  try {
    ...
    const success = await managed.view.webContents.executeJavaScript(
      buildLoginSuccessProbeScript(managed.autofillSuccessSelector, Boolean(managed.interaction)),
      true
    );
    if (success) {
...
async function tryWebProxyAutofill(managed) {
  ...
  try {
    const alreadyLoggedIn = await managed.view.webContents.executeJavaScript(
      buildLoginSuccessProbeScript(session.selectors.success, Boolean(session.selectors.interactive)),
      true
    );
...
async function createWebProxyView(event, win, args) {
  ...
  if (args.safeMode !== undefined && typeof args.safeMode !== "boolean") throw new Error("invalid Web Proxy safe mode");
