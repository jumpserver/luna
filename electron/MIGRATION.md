...
The Vue/Nuxt UI remains shared by Web and Electron. Desktop-only calls must go through
...
| 7     | Replay / transcode          | recording import, local media URLs, H.264 progress            | migrated with Node replay pipeline and optional FFmpeg plugin |

Each workspace is considered migrated when it runs through a host adapter in Electron, keeps a
