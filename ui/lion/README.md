...
开发代理见根目录 `nuxt.config.ts`：Lion 的 API、Token、Health 和 WebSocket 路径默认转发到 Koko（`JMS_KOKO_DEV_URL`，默认 `localhost:5050`）。`JMS_LION_DEV_URL` 仅作为兼容覆盖保留。页面路径由 Client 自己渲染，不转发给 Koko；API/WS 固定走站点根路径 `/koko/lion/*`（不带 `/luna` 前缀）。
