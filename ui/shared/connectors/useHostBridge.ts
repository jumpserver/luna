...
import { HOST_MESSAGE_TYPE } from "~/shared/connectors/types/message";
...
export function createHostBridge() {
  ...
  const onHost = <K extends HostEventType>(type: K, handler: (data: HostMessage) => void) => {
    bus.on(type, handler);
  };

  const offHost = <K extends HostEventType>(type: K, handler?: (data: HostMessage) => void) => {
