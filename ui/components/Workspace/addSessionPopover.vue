...
<template>
  <UPopover v-model:open="open" :content="{ align: 'start', side: 'bottom', sideOffset: 8 }" :ui="{ content: 'p-0' }">
    <UTooltip text="新建连接" :delay-duration="150">
      <button type="button" class="grid size-6 shrink-0 place-items-center rounded-lg text-gray-500 transition-colors hover:bg-black/6 hover:text-gray-800 dark:text-gray-400 dark:hover:bg-white/10 dark:hover:text-white" aria-label="新建连接">
    ...
    <template #content>
      <div class="w-[360px] overflow-hidden rounded-xl bg-default shadow-xl ring-1 ring-black/10 dark:ring-white/12">
        ...
        <div class="max-h-80 min-h-32 overflow-y-auto p-1.5">
          ...
          <button v-for="asset in (search.trim() ? results : recentConnections)" v-else :key="asset.id" type="button" class="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left hover:bg-elevated" @click="selectAsset(asset)">
            <span class="grid size-8 shrink-0 place-items-center rounded-lg bg-elevated"><UIcon name="i-lucide-monitor" class="size-4 text-muted" /></span>
            <span class="min-w-0 flex-1"><span class="block truncate text-sm text-highlighted">{{ asset.name }}</span><span class="block truncate font-ui-mono text-[11px] text-muted">{{ asset.address || asset.platform }}</span></span>
            <UIcon name="i-lucide-chevron-right" class="size-3.5 shrink-0 text-dimmed" />
          </button>
          <div v-if="!loading && (search.trim() ? results : recentConnections).length === 0" class="grid h-24 place-items-center text-xs text-muted">
