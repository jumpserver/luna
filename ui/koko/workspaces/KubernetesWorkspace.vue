        ...
        <div class="app-tree min-h-0 flex-1 overflow-auto px-2 py-2">
          <template v-for="row in treeRows" :key="`${row.kind}:${keyOf(row.node)}`">
            <div class="group relative">
              <button
                class="app-tree-row flex w-full items-center gap-1 rounded-lg pr-1 text-left text-(--app-fg) hover:bg-(--app-hover-soft)"
                :class="[
              ...
              >
                ...
                <img v-if="row.kind === 'asset-root'" :src="kubernetesIconSrc" alt="" class="app-tree-icon shrink-0" />
                <UIcon
                  v-else
                  :name="rowIcon(row)"
                  class="app-tree-icon shrink-0"
          ...
          <div v-if="search.trim() && !treeRows.length" class="px-3 py-2 text-(--app-muted)">
