...
const isPermissionError = computed(() => /forbidden path|not allowed on the scope|permission|operation not permitted/i.test(error.value));
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="min-w-0 space-y-1">
            ...
            <p class="text-xs leading-5 text-muted">
              {{ isPermissionError ? "可以像其他桌面程序一样先做一次本地目录设置，之后文件管理会直接进入你选定的位置。" : error }}
            </p>
            <p v-if="isPermissionError" class="break-all rounded-md bg-default/70 px-2 py-1 font-ui-mono text-[11px] text-muted">
              {{ error }}
            </p>
          </div>
        </div>
    ...
    <UModal v-model:open="setupOpen" title="设置本地文件夹" :ui="{ content: 'max-w-lg' }">
      ...
      <template #footer>
        <div class="flex w-full flex-wrap justify-end gap-2">
          <UButton color="neutral" variant="outline" @click="closeSetup">
            关闭
          </UButton>
