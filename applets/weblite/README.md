...
访问白名单、脚本模式、成功选择器、交互区域选择器和额外 Web 录像属于 XPack 功能，需要有效的企业版许可证。基础代填不受影响。Core 在保存配置和验证连接令牌时检查许可证；无有效许可证时，使用已有高级配置的连接也会被拒绝，不会自动忽略白名单或验证区域。Koko 在创建 Web 录像时再次校验许可证，已有录像仍可完成保存。这些限制需要配套更新 Core、Lina、Luna 和 Koko。
