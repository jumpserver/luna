...
fn create_image_from_bytes(icon_bytes: &[u8], platform: &str) -> Option<Image<'static>> {
    match image::load_from_memory(icon_bytes) {
        Ok(img) => {
            ...
            info!("Loaded custom tray icon for {} ({}x{})", platform, width, height);
            Some(image)
        }
        Err(e) => {
            error!("Failed to load tray icon for {}: {}", platform, e);
            None
        }
    }
}
...
fn load_custom_tray_icon() -> Option<Image<'static>> {
    ...
    {
        let icon_bytes = include_bytes!("../../icons/tray-win.ico");
        create_image_from_bytes(icon_bytes, "Windows")
    }
    
    #[cfg(not(any(target_os = "macos", target_os = "windows")))]
    {
        let icon_bytes = include_bytes!("../../icons/tray-icon.png");
        create_image_from_bytes(icon_bytes, "Linux")
    }
}
