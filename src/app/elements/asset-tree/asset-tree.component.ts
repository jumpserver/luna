...
export class ElementAssetTreeComponent implements OnInit {
  ...
  getRMenuList() {
    ...
    if (cnode.isFavoriteRoot) {
      ...
      const favChecked = favTree
        ? favTree.getCheckedNodes(true).filter(n => !n.isParent)
        : [];
    ...
    this._modal.create({
      ...
      nzOnOk: () => {
        ...
        return this._http
          ...
          .then(() => {
            const msg =
              this._i18n.instant('Create folder') + ' ' + this._i18n.instant('success');
  ...
  onDeleteFolder() {
    ...
    this._modal.confirm({
      nzTitle:
        this._i18n.instant('Delete folder') + ` 「${folderName}」?`,
      ...
      nzOnOk: () => {
        return this._http
          ...
          .then(() => {
            const msg =
              this._i18n.instant('Delete folder') + ' ' + this._i18n.instant('success');
            this._toastr.success(msg, '', { nzClass: 'custom-success-notification' });
            this.refreshFavoriteTree();
          });
      }
